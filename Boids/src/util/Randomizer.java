...
public class Randomizer implements IRandomizer {
	/**
	 * The singleton of this factory
	 */
	public static Randomizer Singleton = new Randomizer();

	/**
	 * Create a factory that can produce all kinds of random data. 
	 */
	private Randomizer() {
	}

	/**
	 * Generates a random location point subject to the constraint that rect.x&lt;=X&lt;=rect.width and rect.y&lt;=Y&lt;=rect.height.
	...
	public Point randomLoc( Rectangle rect) {
		return (new Point( randomInt(rect.x, rect.width), randomInt(rect.y, rect.height)));
	}
	...
	public Point randomVel( Rectangle rect) {
		return (new Point (
				(int) randomChoice(1, -1, 0.5) * randomInt(rect.x, rect.width), 
				(int) randomChoice(1, -1, 0.5) * randomInt(rect.y, rect.height)));
	}
	...
	public Color randomColor() {
		return new Color(randomInt(0,255),randomInt(0,255),randomInt(0,255),210);
	}
